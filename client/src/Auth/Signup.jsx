  ...
  const handleSubmit =async (e) => {
    ...
    try{
      ...
      if(res.ok){
        ...
        alert(`Error:${errdata.message|| "Signup failed"}`)
      }
    }catch(err){
  ...
  return (
    <div
      ...
      <p className="mt-2 max-w-sm text-sm text-neutral-600 dark:text-neutral-300">
        Login to ShareSphere if you can because we don&apos;t have a login flow
        yet
      </p>
...
const BottomGradient = () => {
  return (
    <>
      <span
        className="absolute inset-x-0 -bottom-px block h-px w-full bg-linear-to-r from-transparent via-cyan-500 to-transparent opacity-0 transition duration-500 group-hover/btn:opacity-100" />
      <span
        className="absolute inset-x-10 -bottom-px mx-auto block h-px w-1/2 bg-linear-to-r from-transparent via-indigo-500 to-transparent opacity-0 blur-sm transition duration-500 group-hover/btn:opacity-100" />
    </>
  );
};
